...
韓國食品醫藥品安全處 (MFDS, 식품의약품안전처) 藥品資料載入模組。
...
def load_fda_drugs(filepath: Optional[Path] = None) -> pd.DataFrame:
    ...
    Args:
        filepath: JSON 檔案路徑，預設為 data/raw/kr_fda_drugs.json
    ...
    if filepath is None:
        filepath = Path(__file__).parent.parent.parent.parent / "data" / "raw" / "kr_fda_drugs.json"

    if not filepath.exists():
        raise FileNotFoundError(
            f"找不到藥品資料: {filepath}\n"
            f"請先執行以下步驟：\n"
            f"1. 從 MFDS 의약품통합정보시스템 下載資料\n"
            f"   https://nedrug.mfds.go.kr/\n"
            f"2. 放到 data/raw/ 目錄\n"
