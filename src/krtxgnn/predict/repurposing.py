...
def find_repurposing_candidates(
    ...
    # 建立藥品的現有適應症（向量化操作）
    local_diseases_df = indication_mapping_df[
        indication_mapping_df["disease_name"].notna()
    ][["license_id", "disease_name"]].copy()
    local_diseases_df["disease_lower"] = local_diseases_df["disease_name"].str.lower()
    local_drug_diseases = local_diseases_df.groupby("license_id")["disease_lower"].apply(set).to_dict()
    ...
    # 取得唯一的 (license_id, normalized_ingredient) 組合
    ...
    for _, row in unique_pairs.iterrows():
        ...
        # 取得現有適應症
        local_diseases = local_drug_diseases.get(license_no, set())
        ...
        for disease in kg_diseases:
            ...
            # 快速檢查是否已存在
            is_new = all(
                local_d not in disease_lower and disease_lower not in local_d
